...
//
const my  = 'y-BUILD-sysDB.js';
const ver = `utils/${my} v191015.01`;
const yko = require('../yko/CORE.js');
const Y = new yko ();
//
const KEYS = Y.conf.sysdata.keys;
//
let U;
Y.init().start(my).then(async u => {
  U = u;
  let BOX;
  await U.box.any(KEYS.type, KEYS).then(db=> BOX = db);
	if (BOX.isNew() || ! BOX.get('DontUpdate')) {
		BOX.inport( value() );
		Y.tr( BOX.isNew() ? 'New setup !!' : '... Updated');
		return BOX.prepar();
	} else {
		return Y.tr('No update allowed !!');
	}
}) .then(x=> { if (U) U.finish() });
