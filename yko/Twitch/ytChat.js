...
module.exports = function (y, p) {
	...
	S.conf = P.conf.chat;
	S.im = P.im.chat;
	build_component( Y.debug() && P.im.debug_sleep );
	//
	S.init = () => {
		if (Y.REQ1() == 'Discord') {
			Y.runners(S.connect);
...
let MSG_WRAP;
function build_component (isFake) {
  ...
  const APP = {};
	...
	let [TMP, H] = [{}];
	return isFake ? build_fake(): build_connect();
}
function build_connect () {
  const ON = Y.ON();
	...
	S.connect = async () => {
		...
		Y.tr2('Twitch login status', `"${S.im.loginID}"`
			+ S.im.tagetChannels.map(s=> `'${s}'`).join(', ') );
		Y.tr5('password: ' + S.im.oauthToken);
		return CLIENT;
	};
	S.disconnect = () => {
...
function build_fake () {
	Y.tr2('build_fake');
	P.say = S.say = (ch, msg) => {
		return new Promise( resolve => { Y.c(str); return S });
	};
	S.connect = () => { return S };
}
